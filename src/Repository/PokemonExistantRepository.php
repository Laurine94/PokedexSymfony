...
class PokemonExistantRepository extends ServiceEntityRepository
{
     ...
     * @throws \Doctrine\DBAL\DBALException
     */
    public function getPokemonEnVenteByDresseur($dresseur){
        $conn = $this->getEntityManager()->getConnection();
        $sql = 'SELECT pokemon_existant.id as id ,nom, sexe, niveau, xp,prix FROM pokemon_existant JOIN ref_pokemon_type ON pokemon_existant.pokemon_type_id=ref_pokemon_type.id 
        WHERE dresseur_id != '.$dresseur.' AND prix>0;';
     ...
     * @throws \Doctrine\DBAL\DBALException
     */
    public function getNomByTypeID($idType){
        $conn = $this->getEntityManager()->getConnection();
        $sql = 'SELECT nom FROM ref_pokemon_type JOIN pokemon_existant ON pokemon_existant.pokemon_type_id=ref_pokemon_type.id 
        WHERE pokemon_existant.pokemon_type_id  = '.$idType.';';
     ...
     * @throws \Doctrine\DBAL\DBALException
     */
    public function getPokemonByLieu($lieu){
        $conn = $this->getEntityManager()->getConnection();
    ...
    public function getEvolutionByDresseur($dresseur){
        $conn = $this->getEntityManager()->getConnection();
        $sql = 'SELECT COUNT(pokemon_existant.id) as nb FROM pokemon_existant JOIN ref_pokemon_type ON pokemon_existant.pokemon_type_id=ref_pokemon_type.id WHERE dresseur_id='.$dresseur.' AND ref_pokemon_type.evolution=1;' ;
